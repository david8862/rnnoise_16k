...
def get_lr_scheduler(decay_type, optimizer):
    """
    ...
    elif decay_type == 'lambda':
        lr_decay = 5e-5
        lr_scheduler = optim.lr_scheduler.LambdaLR(optimizer, lr_lambda=lambda x : 1 / (1 + lr_decay * x))
    else:
...
def get_optimizer(optim_type, model, learning_rate, weight_decay):
    ...
    if optim_type == 'adam':
        ...
        adam_betas = [0.8, 0.98]
        adam_eps = 1e-8
        #optimizer = optim.AdamW(model.parameters(), lr=learning_rate, betas=adam_betas, eps=adam_eps)
        optimizer = optim.AdamW(filter(lambda p: p.requires_grad, model.parameters()), lr=learning_rate, betas=adam_betas, eps=adam_eps)
